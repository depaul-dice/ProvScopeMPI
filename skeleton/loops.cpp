...
void fillIloopHeaders(
        cfg *g, std::map<node *, node *>& iloopHeaders) {
    set<node *> visited{};
    // initialization
    map<node *, unsigned> dfspPos{};
...
void printHeaders(const map<node *, node *>& iloopHeaders) {
    for(auto& p: iloopHeaders) {
        cerr << p.first->name 
            << " -> " 
            << (p.second == nullptr ? "nullptr" : p.second->name) 
            << endl;
    }
}
...
void loopNode::print(
    ...
    for(auto &c: this->children) {
        ...
        edges += "root  -> " + childEntryNameReplaced + ";\n";
...
string loopNode::__print(ofstream& file) {
    string edges = "";
    string entryNameReplaced = replaceAll(entry->name);
    file << entryNameReplaced << " [label=\"" << entry->name << "\\l";
    for(auto &n: nodes) {
        file << n->name << "\\l";
    }
    file << "\"];\n";
