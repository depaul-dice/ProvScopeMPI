...
namespace {
    struct VectorAllocInitializer {
        VectorAllocInitializer() {
            optimizeVectorAllocation();
        }
    } vectorAllocInitializer;
}
 ...
 */
vector<ElementPtr> __makeHierarchyLoop(
...
element::element(
        ...
        string& funcname,
        bool isLoop) :
    funcs(vector<vector<ElementPtr>>() ),
    funcname(funcname),
...
string element::bb() const {
    try {
        ...
        // 如果有异常，返回空字符串
        fprintf(stderr, "ERROR: Exception in element::bb()\n");
        return string();
    }
}
...
vector<ElementPtr> __makeHierarchyLoop(
        ...
        loopNode *currloop,
        int iterationCnt) {
    vector<ElementPtr> iterations, curriter;
    ElementPtr eptr;
    bool isEntry = false,
         isExit = false;
    string bbname,
 ...
 */
vector<ElementPtr> makeHierarchy(
        vector<vector<string>>& traces, unsigned long& index) {
    vector<ElementPtr> functionalTraces;
    ...
    do {
        ...
        while(!isExit
                && index < traces.size()
                && traces[index][1] == "entry") {
            vector<ElementPtr> childTraces = makeHierarchy(traces, index);
            if(eptr->funcs.empty()) {
                eptr->funcs.push_back(vector<ElementPtr>());
...
void addHierarchy(
    ...
    while(index < traces.size()) {
         ...
         * Q: why are we looking at parents here?
         */
        ...
        if(traces[index - 1][1] == "exit") {
            ...
            /*     cerr << "currloop: " << *currloop << endl; */
            /*     cerr << "curr: " << *curr << endl; */
            /*     printsurface(parent->funcs.back()); */
            /* } */
            if(currloop->entry == bb) {
                 ...
                 * if the parent is the virtual node of this loop,
                 * you need to make another virtual node for this iterations
...
void appendTraces(
    ...
    if(traceType == TraceType::RECORD) {
        ...
        if(recordTracesRaw.size() > lastProcessedSize) {
            appendTraces(loopTrees, recordTracesRaw, recordTraces);
            lastProcessedSize = recordTracesRaw.size();
        }
    } else if (traceType == TraceType::REPLAY) {
        static size_t lastProcessedSize = 0;

        if(replayTracesRaw.size() > lastProcessedSize) {
            appendTraces(loopTrees, replayTracesRaw, replayTraces);
            lastProcessedSize = replayTracesRaw.size();
        }
    }
}

static string getLastNodes(vector<ElementPtr>& traces) {
    MPI_ASSERT(traces.size() > 0);
    stringstream ss;
    ElementPtr curr = traces.back();
    int funcId = 0;
    do {
        ss << '/' << funcId << ':' << curr->bb();
        if(curr->loopIndex != -1) {
...
void getProperLastInd(size_t& lastInd, ElementPtr curr) {
...
static pair<size_t, size_t> __greedyalignmentOnline(
        vector<ElementPtr>& original,
        vector<ElementPtr>& reproduced,
        vector<pair<ElementPtr, ElementPtr>>& aligned,
        size_t& initI,
        size_t& initJ,
        const int& rank,
        bool& isAligned,
...
deque<shared_ptr<lastaligned>> greedyalignmentOnline(
        ...
        bool& isAligned,
        size_t& lastInd,
        ElementPtr originalParent,
        ElementPtr reproducedParent) {
     ...
     */
    vector<pair<ElementPtr, ElementPtr>> aligned =
        vector<pair<ElementPtr, ElementPtr>>();
 ...
 */
vector<ElementPtr> getCurrNodesByIndex(unsigned long index) {
    vector<ElementPtr> ret;
    if(index == 0) {
        return ret;
    }
    ...
    string functionName = (*raw)[i-1][0];
    for(auto& e : *curr) {
        if(e->funcname == functionName) {
            ret.push_back(e);
        }
    }
